...
export default class DayView extends React.PureComponent {
  ...
  _renderLines() {
    ...
    return range(start, end + 1).map((i, index) => {
      ...
      const InterbalHour = [
       ...
       <TouchableNativeFeedback onPress={() => onPressHour({...value, minute: 0, date: this.props.date})}    key={`touchOne${i}`}>
        <View
          key={`lineHalf${i}`}
          style={[
            {
        ...
        i === start ? null : (
          <View
            key={`line${i}`}
            style={[styles.line,
              { marginLeft: 15, top: offset * index, width: width - 20, marginTop:4  }]}
          />
        ),
        <View
          key={`lineHalf${i}`}
          style={[
            styles.line,
            { marginLeft: 15, top: offset * (index + 0.5), width: width - 20,  marginTop:4   },
          ]}
        />,
        <TouchableNativeFeedback  onPress={() => onPressHour({...value, minute: 30, date: this.props.date, time:this._formatHour({format24h, hour:i, start, half:':30'})})}     key={`touch${i}`}>
          <View
            key={`lineHalf${i}`}
            style={[
              {
  ...
  _renderEvents() {
    ...
    let events = packedEvents.map((event, i) => {
      ...
      return (
        <View key={i} style={[styles.event, style, event.color && eventColor, {marginTop:4, marginLeft: 15}]}>
